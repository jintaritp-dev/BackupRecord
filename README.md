...
- **หน้า dashboard อ่านอย่างเดียว** — มีหน้า `/add` แยกสำหรับกรอกข้อมูล ตั้งรหัสกันเพิ่มได้ถ้าต้องการ
...
รีสตาร์ทแล้วจะมีปุ่ม **"+ เพิ่มรายการ"** บน dashboard
และ `/healthz` จะขึ้น `"writes":"on (no password)"` — เพราะยังไม่ได้ตั้งรหัส ดูหัวข้อถัดไป

### รหัสสำหรับกดบันทึก (ตัวเลือก)

เพิ่ม `WRITE_PASSWORD` ใน `.env` เมื่อต้องการให้ต้องกรอกรหัสก่อนบันทึก

```
WRITE_PASSWORD=รหัสที่ทีมใช้ร่วมกัน
```

| ตั้งค่า | พฤติกรรม |
|---|---|
| เว้นว่าง (ค่าเริ่มต้น) | ใครเปิดหน้า `/add` ได้ก็เพิ่มรายการได้ หน้าเว็บจะขึ้นแบนเนอร์บอกไว้ |
| ใส่ค่า | ต้องกรอกรหัสก่อนบันทึก ผิดครบ 10 ครั้งล็อก IP นั้น 5 นาที |

**ถ้าไม่ตั้งรหัส ตารางนี้เชื่อได้เท่ากับที่เชื่อคนในเครือข่ายได้** — ใครก็เพิ่มแถวเข้ามาได้
ซึ่งอาจรับได้ถ้า LAN เป็นเครือข่ายออฟฟิศที่ปิดอยู่แล้ว แต่ควรรู้ตัวว่ากำลังแลกอะไร
เปิดกลับได้ทุกเมื่อโดยเติมค่าเดียวใน `.env` แล้วรีสตาร์ท ไม่ต้องแก้โค้ด

ถ้าตั้งรหัส: รหัสตัวเดียวใช้ร่วมกันทั้งทีม ไม่มีชื่อผู้ใช้ ระบบจึงไม่รู้ว่าใครกดบันทึก —
ฟอร์มมีช่อง "ผู้ทำ" ให้กรอกแทน และเพราะยังไม่มี HTTPS รหัสจึงวิ่งเป็น plain text ใน LAN
กันคนนอกทีมกับกันกดพลาดได้จริง แต่**ไม่ได้กันคนที่ดักจับ traffic ในเครือข่ายได้**
เบราว์เซอร์จะจำรหัสไว้ใน localStorage — ถ้าเป็นเครื่องรวมที่คนอื่นใช้ด้วย ควรล้างค่าหลังใช้
...
- หน้า `/add` เขียนข้อมูลได้ — ตัดสินใจเรื่องรหัสด้วย ดู [รหัสสำหรับกดบันทึก](#รหัสสำหรับกดบันทึก-ตัวเลือก)
...
| กดบันทึกแล้วขึ้น "กรอกรหัสผิดหลายครั้งเกินไป" | ผิดครบ 10 ครั้งจะล็อก IP นั้น 5 นาที รอแล้วลองใหม่ |
