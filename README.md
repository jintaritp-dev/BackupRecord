...
- **หน้า dashboard อ่านอย่างเดียว** — มีหน้า `/add` แยกสำหรับกรอกข้อมูล ป้องกันด้วยรหัสผ่าน
...
ต้องตั้งสองค่านี้เพิ่มใน `.env` ขาดค่าใดค่าหนึ่งก็ยังปิด
...
### ข้อจำกัดของรหัสผ่านนี้

รหัสตัวเดียวใช้ร่วมกันทั้งทีม ไม่มีชื่อผู้ใช้ ระบบจึงไม่รู้ว่าใครกดบันทึก — ฟอร์มมีช่อง "ผู้ทำ" ให้กรอกแทน
และเพราะยังไม่มี HTTPS รหัสจึงวิ่งเป็น plain text ใน LAN

มันกันคนนอกทีมกับกันกดพลาดได้จริง แต่**ไม่ได้กันคนที่ดักจับ traffic ในเครือข่ายได้**
ถ้าต้องการระดับนั้นให้วางไว้หลัง reverse proxy ที่มี HTTPS

กรอกรหัสผิดครบ 10 ครั้งจะล็อก IP นั้นไว้ 5 นาที (กันเดารหัสอัตโนมัติ)
เบราว์เซอร์จำรหัสไว้ใน localStorage เพื่อไม่ต้องกรอกซ้ำ — ถ้าเป็นเครื่องรวมที่คนอื่นใช้ด้วย ควรล้างค่าหลังใช้
...
- หน้า `/add` มีรหัสกันไว้ แต่รหัสวิ่งเป็น plain text — ดู [ข้อจำกัดของรหัสผ่านนี้](#ข้อจำกัดของรหัสผ่านนี้)
...
| หน้า `/add` ขึ้นว่าปิดอยู่ | ขาด `WRITE_PASSWORD` หรือ `DATABASE_URL_WRITE` ใน `.env` — แบนเนอร์บอกว่าขาดตัวไหน |
