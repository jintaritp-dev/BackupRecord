...
- **หน้า `/add` ไม่มีรหัสกัน** ใครเปิด URL ได้ก็เพิ่มรายการได้ — ขั้น 4 (ไฟร์วอลล์)
  จึงเป็นด่านเดียวที่กันคนนอก ทำให้แน่น
...
| `"writes":"on"` | หน้า `/add` เปิดใช้งานได้ |
...
> ใครเปิด URL ได้ก็เห็นว่าบริษัทเก็บ backup อะไรไว้ที่ไหนทั้งหมด — และเพิ่มรายการเข้าตารางได้ด้วย
...
เปิดหน้า `/add` แล้วกรอก — ต้องตั้ง `DATABASE_URL_WRITE` ไว้ก่อน (ดูขั้น 3.4)
...
- [ ] รู้แล้วว่าหน้า `/add` ไม่มีรหัสกัน — ใครใน subnet ที่เปิดไฟร์วอลล์ไว้ก็เพิ่มรายการได้
